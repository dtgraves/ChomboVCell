...
GeometryService::InOut GeometryService::InsideOutside(const Box&           a_region,
                                                      ...
                                                      const Real&          a_dx) const
{
  bool regular = isRegular(a_region, a_domain, a_origin, a_dx);
  bool covered = isCovered(a_region, a_domain, a_origin, a_dx);
  if (regular) return GeometryService::Regular;
  if (covered) return GeometryService::Covered;
