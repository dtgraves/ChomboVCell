...
int threadTest()
{
  ...
  const IntVectSet sharedAndConst = foo;
  ...
  for(int ibox=0; ibox<nbox; ibox++)
    {
      ...
      IntVectSet ivs1(DenseIntVectSet(fbox,true));
      for(IVSIterator ivit(ivs1); ivit.ok(); ++ivit)
        {
      ...
      ivs2 &= inner;
      for(IVSIterator ivit(ivs2); ivit.ok(); ++ivit)
        {
          const IntVect& iv = ivit();
          fab(iv,0) = -ibox;
        }      
      ...
      //#pragma omp critical
      {
        IntVectSet ivs3 = sharedAndConst;
        ivs3 &=  grids.get(dit[ibox]);
        ivs3 |=  grids.get(dit[ibox]);
        for(IVSIterator ivit(ivs3); ivit.ok(); ++ivit)
          {
            const IntVect& iv = ivit();
            fab(iv,0) = 0.5;
          }
