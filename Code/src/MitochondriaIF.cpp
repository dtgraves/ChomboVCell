                        ...
                        const bool     & a_cylinder)
{
  ...
  Real part1 = Max(
                   fabs(x - const1) - r2,
                   D_TERM(0.0, + y*y, + z*z) - r1*r1
                  );

  Real part2 = Min(
                   part1,
                   D_TERM((x-0.3)*(x-0.3), + y*y, + z*z) - r2*r2
                  );
...
SphereDimpleIF1::SphereDimpleIF1()
{
}
...
Real SphereDimpleIF1::value(const RealVect & a_point) const
{
...
BaseIF* SphereDimpleIF1::newImplicitFunction() const
{
  return new SphereDimpleIF1();
}


CylinderDimpleIF1::CylinderDimpleIF1()
{
}
...
Real CylinderDimpleIF1::value(const RealVect & a_point) const
{
...
BaseIF* CylinderDimpleIF1::newImplicitFunction() const
{
  return new CylinderDimpleIF1();
}


MitochondriaIF1::MitochondriaIF1()
{
  // m_dimple = new SphereDimpleIF1();
  m_dimple = new SphereDimpleIF1();
#if 0
  m_dimple2 = new TransformIF(*m_dimple);

  RealVect scale(D_DECL(-1.0,1.0,1.0));
  m_dimple2->scale(scale);

  Real shrink = 0.1/0.9;
  m_dimple2->scale(shrink);

  RealVect offset(D_DECL(0.3,0.0,0.0));
  m_dimple2->translate(offset);

  m_dimple3 = new TransformIF(*m_dimple2);

  m_dimple3->scale(scale);
  m_dimple3->scale(shrink);
  m_dimple3->translate(offset);
#endif
}
...
Real MitochondriaIF1::value(const RealVect & a_point) const
{
  Real x;
  Real y;
  Real z;
...
BaseIF* MitochondriaIF1::newImplicitFunction() const
{
  return new MitochondriaIF1();
}
