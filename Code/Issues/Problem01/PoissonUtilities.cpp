               ...
               const Box&              a_domain)
{
  ...
  Box domlev = a_domain;
  a_boxes[0].push_back(a_domain);

  Box fineDomlev = domlev;
  fineDomlev.refine(a_refRat[0]);

  IntVect fineLo   = fineDomlev.smallEnd();
  IntVect fineHi   = fineDomlev.bigEnd();
  IntVect fineSize = fineDomlev.size();

  IntVect fineLo1 = fineLo;
  IntVect fineHi1 = fineHi;
  fineHi1[0] -= fineSize[0]/2;

  IntVect fineLo2 = fineLo;
  fineLo2[0] += fineSize[0]/2;
  IntVect fineHi2 = fineHi;
  fineHi2[1] -= fineSize[1]/2;

  Box reflev1(fineLo1,fineHi1);
  Box reflev2(fineLo2,fineHi2);

  a_boxes[1].push_back(reflev1);
  a_boxes[1].push_back(reflev2);
}
void
  ...
  for (int ilev = 0; ilev < nlevels; ilev++)
    {
      ...
      EBLevelDataOps::setVal(*(a_scalar[ilev]),a_value);
  ...
  if (!pp.contains("ebis_file"))
    {
      ...
      if (whichgeom == 0)
        ...
        {
          pout() << "plane geometry" << endl;

          RealVect planeNormal;
          vector<Real> planeNormalVect(SpaceDim);
          pp.getarr("plane_normal",planeNormalVect, 0, SpaceDim);
          for (int idir = 0; idir < SpaceDim; idir++)
            {
              planeNormal[idir] = planeNormalVect[idir];
            }

          RealVect planePoint;
          vector<Real> planePointVect(SpaceDim);
          pp.getarr("plane_point",planePointVect, 0, SpaceDim);
          for (int idir = 0; idir < SpaceDim; idir++)
            {
              planePoint[idir] = planePointVect[idir];
            }

          bool inside = true;

          PlaneIF plane(planeNormal,planePoint,inside);

          GeometryShop workshop(plane,verbosity,fineDx);
